...
申请、配置说明缺失或尚未解决的接入问题，请联系 [TikTok_LIVE_Studio_Office@bytedance.com](mailto:TikTok_LIVE_Studio_Office@bytedance.com)。请提供 LIVE Studio 版本、客户端运行时、最后连接状态和脱敏错误码，不要发送 `secret`、完整 `AUTH` 内容或观众原始数据。申请结果通过邮箱通知，审核时间因申请情况而异。
...
- `49152` 到 `65535` 没有候选端点返回合法的 `SERVER_HELLO`。
- WebSocket 始终打不开。
...
- 扫描器采用有界并发；对整个范围使用长超时串行扫描，看起来会像卡死。
- 客户端没有假定上一次启动使用的端口仍然有效。
