...
LIVE Studio 会在约定的动态范围内绑定一个端口：

```txt
127.0.0.1:49152-65535
```

LIVE Studio 会从 `49152` 开始向上探测，并绑定第一个空闲端口。重启后端口可能变化。请小批量、有上限地并发扫描候选端点，直到收到合法的 `SERVER_HELLO`：
...
不要对 16384 个端口使用长超时串行扫描，也不要一次性全部打开。示例采用有界并发，并在选中合法端点后关闭其余候选 Socket。
